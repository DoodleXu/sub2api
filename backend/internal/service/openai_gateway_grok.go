...
func (s *OpenAIGatewayService) forwardGrokResponses(
	...
	normalizedOriginalModel := strings.ToLower(strings.TrimSpace(originalModel))
	if normalizedOriginalModel == "grok" || normalizedOriginalModel == "grok-latest" {
		rawMapping, _ := account.Credentials["model_mapping"].(map[string]any)
		if _, explicitlyMapped := rawMapping[normalizedOriginalModel]; !explicitlyMapped {
