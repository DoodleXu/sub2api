...
func (s *OpenAIGatewayService) handleOpenAIImagesErrorResponse(
	...
	shouldFailover := shouldDisable || isOpenAIHTTPUpstreamAccessStateError(resp.StatusCode, upstreamMsg, body) ||
		(account.IsOpenAIOAuthLike() && resp.StatusCode == http.StatusTooManyRequests && failoverErr.RetryableOnSameAccount)
	kind := "http_error"
	if shouldFailover {
		kind = "failover"
	}
...
func (s *OpenAIGatewayService) handleOpenAIImagesOAuthNonStreamingResponse(
...
) (OpenAIUsage, int, []string, []byte, error) {
	...
	if len(results) == 0 {
		...
		if textFallbackErr := openAIImagesTextFallbackError(body); textFallbackErr != nil {
			...
			return OpenAIUsage{}, 0, nil, nil, textFallbackErr
		}
		...
		return OpenAIUsage{}, 0, nil, nil, &UpstreamFailoverError{
			StatusCode:             http.StatusBadGateway,
			ResponseBody:           body,
	...
	return usage, len(results), openAIResponsesImageResultSizes(results), responseBody, nil
}
...
func (s *OpenAIGatewayService) handleOpenAIImagesOAuthStreamingResponse(
	...
	if streamInterval <= 0 && keepaliveInterval <= 0 {
		...
		for {
			...
			if processErr != nil {
				return usage, imageCount, imageOutputSizes, firstTokenMs, imageFirstOutputMs, processErr
			}
			if done {
				return usage, imageCount, imageOutputSizes, firstTokenMs, imageFirstOutputMs, nil
			}
			if err == io.EOF {
				break
			}
			if err != nil {
				...
				return usage, imageCount, imageOutputSizes, firstTokenMs, imageFirstOutputMs, err
			}
		}
		if done, processErr := flushData(); processErr != nil {
			return usage, imageCount, imageOutputSizes, firstTokenMs, imageFirstOutputMs, processErr
	...
	for {
		...
		case ev, ok := <-events:
			...
			if ev.err != nil {
				...
				return usage, imageCount, imageOutputSizes, firstTokenMs, imageFirstOutputMs, ev.err
			}
		...
		case <-intervalCh:
			...
			if clientDisconnected {
				return usage, imageCount, imageOutputSizes, firstTokenMs, imageFirstOutputMs, fmt.Errorf("image stream incomplete after timeout")
			}
			...
			return usage, imageCount, imageOutputSizes, firstTokenMs, imageFirstOutputMs, fmt.Errorf("image stream data interval timeout")
		case <-keepaliveCh:
