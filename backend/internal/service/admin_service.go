...
// ChannelCacheInvalidator invalidates channel-derived pricing and model caches.
...
func (s *adminServiceImpl) CreateGroup(ctx context.Context, input *CreateGroupInput) (*Group, error) {
	...
	if codexManifestConfig.Enabled {
...
func (s *adminServiceImpl) ListAccounts(ctx context.Context, page, pageSize int, platform, accountType, status, search string, groupID int64, privacyMode string, sortBy, sortOrder string) ([]Account, int64, error) {
	if s.cfg != nil && s.cfg.RunMode == config.RunModeSimple && groupID > 0 {
		group, err := s.groupRepo.GetByID(ctx, groupID)
		if err != nil {
			return nil, 0, err
		}
...
func (s *adminServiceImpl) UpdateAccount(ctx context.Context, id int64, input *UpdateAccountInput) (*Account, error) {
	...
	if input.Extra != nil {
		...
		for _, key := range []string{
			...
			OpenCodeGoUsageAutoRefreshExtraKey, OpenCodeGoUsageSnapshotExtraKey,
		} {
...
func (s *adminServiceImpl) BulkUpdateAccounts(ctx context.Context, input *BulkUpdateAccountsInput) (*BulkUpdateAccountsResult, error) {
	...
	if input.RateMultiplier != nil {
		...
		syncEnabledCount := 0
		for _, account := range cachedTargets {
			if account != nil && upstreamBillingRateSyncEnabled(account) {
				syncEnabledCount++
			}
		}
