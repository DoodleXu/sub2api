...
// testCNProviderChatCompletionsConnection tests a CN provider API key through
// its OpenAI-compatible Chat Completions endpoint. Anthropic and Responses
// protocol accounts are routed by the normal platform-specific paths above.
...
func (s *AccountTestService) testOpenAIImageOAuth(c *gin.Context, ctx context.Context, account *Account, modelID, prompt string) error {
	...
	// 与真实转发一致：先做账号模型映射，再决定走 Codex 原生图片端点还是
	// Responses 图片工具。
	...
	// 直连端点返回原生 Images JSON，Responses 驱动返回 SSE；两者解析方式不同。
