...
const (
	...
	defaultOpenAISchedulingUSDToCNYRate        = 7.2
)
...
type OpenAIAccountScheduleRequest struct {
	...
	// RequireCompact only filters the legacy /responses/compact capability.
	// Native remote compaction v2 uses the ordinary Responses capability.
	RequireCompact        bool
...
func (s *defaultOpenAIAccountScheduler) Select(
	...
	if previousResponseID != "" && NormalizeOpenAICompatiblePlatform(req.Platform) == PlatformOpenAI &&
		...
		if selection != nil && selection.Account != nil {
			if s.service.openAIGroupRequiresPrivacySet(ctx, req.GroupID) && !selection.Account.IsPrivacySet() {
	...
	if req.GuardianParentAccountID > 0 {
		...
		if selection != nil && selection.Account != nil && req.RequirePrivacySet && !selection.Account.IsPrivacySet() {
			if selection.ReleaseFunc != nil {
				selection.ReleaseFunc()
			}
			selection = nil
...
func (s *defaultOpenAIAccountScheduler) selectBySessionHash(
	...
	if req.RequirePrivacySet && !account.IsPrivacySet() {
		// Privacy eligibility is group-scoped. Keep the shared sticky binding so
		// a transient policy choice does not erase affinity for other groups.
		return nil, false, nil
	}
	...
	if account == nil || (req.RequirePrivacySet && !account.IsPrivacySet()) || !s.service.openAIAccountMatchesSchedulingGroup(account, req.GroupID) || !s.isAccountRequestCompatible(ctx, account, req) || !s.isAccountTransportCompatible(account, req.RequiredTransport) {
		clearBinding()
		return nil, false, nil
	}
...
func (s *defaultOpenAIAccountScheduler) buildOpenAIAccountLoadPlan(
	...
	if req.UseUpstreamTokenCost && weights.UpstreamCost > 0 {
		...
		upstreamCostFactors = openAIUpstreamCostFactors(
			accounts,
			now,
			s.service.openAIOAuthSchedulingRateMultiplier(ctx),
			s.service.openAISchedulingUSDToCNYRate(ctx),
		)
...
func (s *defaultOpenAIAccountScheduler) isAccountRequestCompatibleReason(ctx context.Context, account *Account, req OpenAIAccountScheduleRequest) (bool, string) {
	...
	// 分组利润控制：先完成账号级利润资格判断，再观察代理隔离状态，
	// 避免把本就不满足利润门的账号计入请求级代理隔离降级信号。
	if vetoed, reason := openAIProfitControlVetoReason(ctx, account); vetoed {
		return false, reason
	}
	// Evaluate proxy quarantine after all account-level eligibility gates. This
	// ensures the request-scoped observation only records proxies whose accounts
	// could otherwise serve the request.
...
func (s *OpenAIGatewayService) openAISchedulingUSDToCNYRate(ctx context.Context) float64 {
	return s.openAIAdvancedSchedulerRuntimeSettings(ctx).schedulingUSDToCNYRate
}
...
func openAIAdvancedSchedulerRuntimeSettingKeys() []string {
	keys := []string{
		...
		SettingKeyOpenAISchedulingUSDToCNYRate,
...
func (s *OpenAIGatewayService) selectAccountWithScheduler(
	...
	firstPassCtx, quarantineObservation := withOpenAIProxyStreamQuarantineObservation(ctx)
	selection, decision, err := s.selectAccountWithSchedulerOnce(firstPassCtx, groupID, previousResponseID, sessionHash, requestedModel, excludedIDs, requiredTransport, requiredCapability, requiredImageCapability, requireCompact, platform, previousResponseCanMove, useUpstreamTokenCost)
	...
	blocked := quarantineObservation.count()
	if blocked == 0 {
		return selection, decision, err
	}
	s.logOpenAIProxyStreamQuarantineFailOpen(requestedModel, blocked)
	return s.selectAccountWithSchedulerOnce(withOpenAIProxyStreamQuarantineBypass(firstPassCtx), groupID, previousResponseID, sessionHash, requestedModel, excludedIDs, requiredTransport, requiredCapability, requiredImageCapability, requireCompact, platform, previousResponseCanMove, useUpstreamTokenCost)
}
...
func (s *OpenAIGatewayService) ReportOpenAIAccountScheduleResult(accountArg any, model string, success bool, firstTokenMs *int, observedErr ...error) bool {
	var account *Account
	switch value := accountArg.(type) {
	case *Account:
		account = value
	case int64:
		account = &Account{ID: value, Platform: PlatformOpenAI}
	case int:
		account = &Account{ID: int64(value), Platform: PlatformOpenAI}
	}
	if account == nil {
		return false
	}
...
func (s *RateLimitService) BuildOpenAIAccountSchedulerScoreSnapshot(
	...
	return buildOpenAIAccountSchedulerScoreSnapshot(
		...
		gateway.openAISchedulingUSDToCNYRate(ctx),
	)
}
...
func BuildOpenAIAccountSchedulerScoreSnapshot(
	...
	return buildOpenAIAccountSchedulerScoreSnapshot(
		accounts,
		loadMap,
		gateway.openAIWSSchedulerWeights(),
		false,
