...
// SelectCodexModelsAccount preserves the fork's OAuth-only selector used by
// callers that explicitly require a ChatGPT backend token. The public manifest
// handler uses the broader upstream selector so custom API-key upstreams remain
// supported.
...
func (s *OpenAIGatewayService) FetchCodexModelsManifest(ctx context.Context, account *Account, clientVersion, ifNoneMatch string) (*CodexModelsManifest, error) {
	...
	if credAccount == nil {
		return nil, infraerrors.New(http.StatusBadGateway, "OPENAI_CODEX_MODELS_ACCOUNT_TYPE_UNSUPPORTED", "credential account is unavailable")
	}
