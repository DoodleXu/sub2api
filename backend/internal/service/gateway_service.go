...
const (
	...
	defaultMaxLineSize      = 128 * 1024 * 1024
...
# Tone and style
 ...
 - Do not use a colon before tool calls. Your tool calls may not be shown directly in the output, so text like "Let me read the file:" followed by a read tool call should just be "Let me read the file." with a period.`
	...
	compositeModelOwnershipCachePrefix     = "composite-model-ownership|"
...
// hasObservedTokens reports whether streaming observed any upstream-metered usage.
...
func (s *GatewayService) routingAccountIDsForRequest(ctx context.Context, groupID *int64, requestedModel string, platform string) []int64 {
	if groupID == nil || requestedModel == "" || !modelRoutingAppliesToPlatform(platform, PlatformComposite) {
		return nil
	}
...
// Fable 5 refuses the generic CLI expansion while accepting the native billing
// and Claude Code identity blocks. Original client instructions are still moved
// into the message history by rewriteSystemForNonClaudeCodeWithPromptBlocks.
...
func rewriteSystemForNonClaudeCodeWithPromptBlocks(body []byte, system any, expansionPrompt string, blocksConfig string) []byte {
	...
	// 1. 提取原始 system prompt 文本及最后一个客户端缓存断点。
...
// systemHasBillingAttributionBlock detects real Claude Code traffic whose
// User-Agent was replaced by an intermediate API gateway while its stable
// billing attribution block remained in the request body.
	...
	for attempt := 1; attempt <= maxRetryAttempts; attempt++ {
		...
		if err != nil {
			...
			return nil, &UpstreamFailoverError{
				...
				Reason:       GatewayFailureReason("upstream_request_error"),
			}
		}
...
func (s *GatewayService) forwardAnthropicAPIKeyPassthroughWithInput(
	...
	for attempt := 1; attempt <= maxRetryAttempts; attempt++ {
		...
		if err != nil {
			...
			return nil, &UpstreamFailoverError{StatusCode: http.StatusBadGateway, ResponseBody: []byte(safeErr), Reason: GatewayFailureReason("upstream_request_error")}
		}
...
func (s *GatewayService) buildUpstreamRequest(ctx context.Context, c *gin.Context, account *Account, body []byte, token, tokenType, modelID string, reqStream bool, mimicClaudeCode bool) (*http.Request, []byte, error) {
	body = stripDeferredToolCacheControl(body)
	body = clampOllamaCloudUpstreamMaxTokensForModel(account, body, modelID)
...
// partialStreamUsageResult preserves already observed upstream usage when a stream
// terminates before its normal terminal event. Failover errors deliberately return
// nil so a successful retry cannot be billed twice.
