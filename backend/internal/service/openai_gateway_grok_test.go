...
func TestBuildGrokResponsesRequestHonorsOAuthCustomBaseURL(t *testing.T) {
	t.Parallel()
...
func TestForwardGrokMediaOAuthImageToVideoUsesOfficialAPIForLargeBody(t *testing.T) {
	...
	account := healthyGrokOAuthGatewayTestAccount(66, "oauth-access-token")
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusOK,
		Header: http.Header{
			"Content-Type": []string{"application/json"},
...
func TestForwardAsAnthropicForGrokFreePureFunctionToolsDoNotInjectSearch(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	require.False(t, gjson.GetBytes(upstream.lastBody, `tools.#(type=="web_search")`).Exists())
	require.False(t, gjson.GetBytes(upstream.lastBody, `tools.#(type=="x_search")`).Exists())
...
func grokMessagesSSECompletedResponse(responseID string, cachedTokens int) *http.Response {
	body := strings.Join([]string{
		fmt.Sprintf(`data: {"type":"response.completed","response":{"id":%q,"object":"response","model":"grok-4.5","status":"completed","output":[{"type":"message","id":"msg_1","role":"assistant","status":"completed","content":[{"type":"output_text","text":"ok"}]}],"usage":{"input_tokens":5,"output_tokens":2,"total_tokens":7,"input_tokens_details":{"cached_tokens":%d}}}}`, responseID, cachedTokens),
		"",
		"data: [DONE]",
		"",
	}, "\n")
