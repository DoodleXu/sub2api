...
// TestPassthroughIngressCallsBeforeTurnAndReportsStart 保证 ws_v2 透传入口在
// 每个 turn 开始时复核定价与利润，并在 AfterTurn 前报告同一 turn 的开始时刻。
func TestPassthroughIngressCallsBeforeTurnAndReportsStart(t *testing.T) {
	...
	require.Positive(t, gotBefore, "透传 ingress 应在 turn 开始时调用 BeforeTurn")
