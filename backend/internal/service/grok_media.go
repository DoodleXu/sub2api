...
func GrokMediaVideoRequestSessionHash(requestID string, ownerIDs ...int64) string {
	...
	userID, apiKeyID := ownerIDs[0], ownerIDs[1]
...
func (s *OpenAIGatewayService) BindGrokMediaVideoRequestAccount(
	...
	var accountID int64
	if len(ids) == 1 {
		sessionHash, accountID = GrokMediaVideoRequestSessionHash(requestID), ids[0]
	} else if len(ids) >= 3 {
		sessionHash, accountID = GrokMediaVideoRequestSessionHash(requestID, ids[0], ids[1]), ids[2]
	}
...
func (s *OpenAIGatewayService) ForwardGrokMedia(
	...
	if endpoint.RequiresRequestBody() && gjson.ValidBytes(body) {
		mappedModel := strings.TrimSpace(account.GetMappedModel(requestInfo.Model))
		// The fork's default Grok mapping keeps the legacy edit alias for text
		// compatibility, while the media endpoint accepts the explicit model ID.
		// Endpoint normalization handles the generic grok-imagine alias first;
		// preserve an explicit edit ID unless an operator configured a mapping.
		if requestInfo.Model == "grok-imagine-edit" && !hasConfiguredGrokModelMapping(account) {
			mappedModel = requestInfo.Model
		}
		if mappedModel != "" {
...
func hasConfiguredGrokModelMapping(account *Account) bool {
