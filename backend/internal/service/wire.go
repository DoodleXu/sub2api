...
// ProvideBatchImageCleanupService creates and starts the periodic image cleanup worker.
...
func ProvideDashboardAggregationService(repo DashboardAggregationRepository, timingWheel *TimingWheelService, lockCache LeaderLockCache, db *sql.DB, dashboardCache DashboardStatsCache, cfg *config.Config, settingRepo SettingRepository) *DashboardAggregationService {
	svc := NewDashboardAggregationService(repo, timingWheel, cfg)
	svc.settingRepo = settingRepo
	svc.SetLeaderLock(lockCache, db)
	svc.SetDashboardCache(dashboardCache)
	svc.Start()
	return svc
}
...
// ProvideOpenAIGatewayService wires the durable Grok async-video task store
// without changing the public constructor used by focused unit tests.
func ProvideOpenAIGatewayService(
	...
	svc.SetGrokVideoTaskRepository(grokVideoTaskRepo)
	return svc
}
...
// ProvideAuditLogService creates the audit log service and starts its
// asynchronous writer and retention cleanup loop.
