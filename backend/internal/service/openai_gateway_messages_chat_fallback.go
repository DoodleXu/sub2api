...
func (s *OpenAIGatewayService) forwardAnthropicViaRawChatCompletions(
	...
	billingModel := resolveOpenAIChatForwardModel(account, anthropicReq.Model, defaultMappedModel)
	...
	if resp.StatusCode >= 400 {
		respBody, upstreamMsg := s.readOpenAIUpstreamError(ctx, account, resp)
