...
func (s *OpenAIGatewayService) ForwardAsAnthropic(
	...
	// 2. Model mapping
	billingModel := resolveOpenAIChatForwardModel(account, normalizedModel, defaultMappedModel)
	...
	if resp.StatusCode >= 400 {
		...
		return s.handleAnthropicErrorResponse(ctx, resp, c, account, billingModel)
	}
...
func (s *OpenAIGatewayService) handleAnthropicErrorResponse(
	...
	return s.handleCompatErrorResponse(ctx, resp, c, account, writeAnthropicError, requestedModel...)
}
...
func (s *OpenAIGatewayService) handleAnthropicBufferedStreamingResponse(
	...
	finalResponse, usage, acc, err := s.readOpenAICompatBufferedTerminal(resp, c, "openai messages buffered", requestID, redactSensitiveBody)
	if err != nil {
	...
	for {
		...
		case ev, ok := <-events:
			...
			if ev.err != nil {
				...
				return nil, usage, acc, &openAICompatBufferedReadError{cause: safeErr}
			}
