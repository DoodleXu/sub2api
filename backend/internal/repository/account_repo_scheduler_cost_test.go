...
func TestPreserveCachedAccountCostStatsRecomputesAfterAccountCostChange(t *testing.T) {
	...
	mock.ExpectQuery(regexp.QuoteMeta("FROM usage_account_cost_totals")).
		WillReturnRows(sqlmock.NewRows([]string{"account_id", "published_account_cost", "published_standard_account_cost", "published_initialized", "complete"}).AddRow(1, 120.0, 100.0, true, true))
