...
//
// succeeded 是新写入的明确结果；历史行保持 NULL 并回退 actual_cost，以免迁移
// 篡改既有报表。零价/免费成功请求因此不会被统计为失败。
// 配合 `FROM usage_logs ul` JOIN 查询使用。
const usageLogSuccessFilterUL = "COALESCE(ul.succeeded, ul.actual_cost > 0)"
...
func appendUsageLogBillingModeWhereConditionWithAlias(conditions []string, args []any, billingMode string, alias string) ([]string, []any) {
	...
	billingModeColumn := usageLogQualifiedColumn(alias, "billing_mode")
	imageCountColumn := usageLogQualifiedColumn(alias, "image_count")
...
func appendUsageLogBillingModeQueryFilter(query string, args []any, billingMode string, alias string) (string, []any) {
	conditions, nextArgs := appendUsageLogBillingModeWhereConditionWithAlias(nil, args, billingMode, alias)
	if len(conditions) == 0 {
...
func appendUsageLogUpstreamModelMismatchWhereCondition(
	conditions []string,
	...
	column := usageLogQualifiedColumn(alias, "upstream_model_mismatch")
	if *upstreamModelMismatch {
		return append(conditions, column+" IS TRUE")
	}
	return append(conditions, column+" IS FALSE")
}

func appendUsageLogUpstreamModelMismatchQueryFilter(query string, upstreamModelMismatch *bool, alias string) string {
	conditions := appendUsageLogUpstreamModelMismatchWhereCondition(nil, upstreamModelMismatch, alias)
	if len(conditions) == 0 {
		return query
	}
	return query + " AND " + conditions[0]
}

func usageLogQualifiedColumn(alias string, column string) string {
	if strings.TrimSpace(alias) == "" {
		return column
	}
	return strings.TrimSpace(alias) + "." + column
}

func usageLogModelDimensionExpression(source string, alias string) string {
	requestedModel := usageLogQualifiedColumn(alias, "requested_model")
...
func appendUsageLogModelQueryFilter(query string, args []any, model string, source string, alias string) (string, []any) {
	if strings.TrimSpace(model) == "" {
		return query, args
	}
	query += fmt.Sprintf(" AND %s = $%d", usageLogModelDimensionExpression(source, alias), len(args)+1)
...
func prepareUsageLogInsert(log *service.UsageLog) usageLogInsertPrepared {
	...
	imageFirstOutput := nullInt(log.ImageFirstOutputMs)
	...
	if log.UpstreamModelMismatch != nil {
		upstreamModelMismatch = sql.NullBool{Bool: *log.UpstreamModelMismatch, Valid: true}
	}
	succeeded := true
	if log.Succeeded != nil {
...
func (r *usageLogRepository) GetUserUsageTrend(ctx context.Context, startTime, endTime time.Time, granularity string, limit int) (results []UserUsageTrendPoint, err error) {
	...
	if canCheckCoverage {
		...
		// Daily tables intentionally cover complete days only. When a day-level
		// request includes the current unfinished day, group the covered hourly
		// buckets by day instead of returning an empty section or reading raw logs.
		if coverageKind == dashboardUserAggregateCoverageDaily {
	...
	// Dashboard widgets must never fall back to grouping raw usage_logs. Missing
	// aggregate coverage is represented as an empty optional section instead.
...
func (r *usageLogRepository) GetUserSpendingRanking(ctx context.Context, startTime, endTime time.Time, limit int) (result *UserSpendingRankingResponse, err error) {
	...
	if isWholeDashboardDayRange(startTime, endTime) {
		if aggregateEnd, canUseDaily, coverageErr := r.dashboardUserAggregateCoveredEnd(ctx, startTime, endTime, dashboardUserAggregateCoverageDaily); coverageErr != nil {
			return nil, coverageErr
		} else if canUseDaily {
			if aggregated, _, aggregatedErr := r.getUserSpendingRankingFromAggregates(ctx, startTime, aggregateEnd, limit); aggregatedErr != nil {
				if !isUndefinedTableError(aggregatedErr) {
					return nil, aggregatedErr
				}
			} else {
	...
	if aggregateEnd, canUseHourly, coverageErr := r.dashboardUserAggregateCoveredEnd(ctx, startTime, endTime, dashboardUserAggregateCoverageHourly); coverageErr != nil {
		...
		if aggregated, _, aggregatedErr := r.getUserSpendingRankingFromHourlyAggregates(ctx, startTime, aggregateEnd, limit); aggregatedErr != nil {
			if !isUndefinedTableError(aggregatedErr) {
				return nil, aggregatedErr
			}
		} else {
...
func (r *usageLogRepository) dashboardUserAggregateCoveredEnd(ctx context.Context, startTime, endTime time.Time, kind dashboardUserAggregateCoverageKind) (time.Time, bool, error) {
	...
	allowFutureTail := kind == dashboardUserAggregateCoverageHourly
	aggregateEnd, covered := dashboardAggregateCoveredEnd(startTime, endTime, coveredFrom, coveredTo, allowFutureTail)
...
func shouldUseFastUsageLogTotal(filters UsageLogFilters) bool {
	if filters.ExactTotal || filters.UpstreamModelMismatch != nil {
		return false
	}
...
func (r *usageLogRepository) GetUsageTrendWithUsageFilters(ctx context.Context, startTime, endTime time.Time, granularity string, filters usagestats.UsageLogFilters) (results []TrendDataPoint, err error) {
	dateFormat := safeDateFormat(granularity)
...
func (r *usageLogRepository) getModelStatsFromDashboardAggregates(ctx context.Context, startTime, endTime time.Time) (results []ModelStat, err error) {
	aggregateEnd, covered, coverageErr := r.dashboardModelAggregateCoveredEnd(ctx, startTime, endTime)
	if coverageErr != nil {
		return nil, coverageErr
	}
	if !covered {
...
func (r *usageLogRepository) dashboardModelAggregateCoveredEnd(ctx context.Context, startTime, endTime time.Time) (time.Time, bool, error) {
	var coveredFrom, coveredTo time.Time
	query := `
		SELECT model_hourly_aggregated_from, model_hourly_last_aggregated_at
...
// returns the safe aggregate query end. Historical gaps remain unavailable. A
// request whose end is still in the future (the dashboard's inclusive current
// date shape) may use the covered realtime prefix without querying raw logs.
func dashboardAggregateCoveredEnd(startTime, endTime, coveredFrom, coveredTo time.Time, allowFutureTail bool) (time.Time, bool) {
	epoch := time.Unix(0, 0).UTC()
	startUTC := startTime.UTC()
	endUTC := endTime.UTC()
	coveredFromUTC := coveredFrom.UTC()
	coveredToUTC := coveredTo.UTC()
	if !endUTC.After(startUTC) || !coveredToUTC.After(epoch) || startUTC.Before(coveredFromUTC) {
		return time.Time{}, false
	}
	if !endUTC.After(coveredToUTC) {
		return endTime, true
	}
	if !allowFutureTail || !endUTC.After(time.Now().UTC()) || !coveredToUTC.After(startUTC) {
		return time.Time{}, false
	}
...
// GetAllGroupUsageSummary returns today's, yesterday's, and cumulative actual_cost
// for every group using the persisted daily rollups plus the unpublished raw tail.
...
func (r *usageLogRepository) GetStatsWithFilters(ctx context.Context, filters UsageLogFilters) (*UsageStats, error) {
	...
	runEndpoints := func(c context.Context) {
		res, err := r.getEndpointStatsByColumnWithFilters(c, "inbound_endpoint", start, end, filters.UserID, filters.APIKeyID, filters.AccountID, filters.GroupID, filters.Model, filters.RequestType, filters.Stream, filters.BillingType, filters.UpstreamModelMismatch)
		if err != nil {
	...
	runUpstream := func(c context.Context) {
		res, err := r.getEndpointStatsByColumnWithFilters(c, "upstream_endpoint", start, end, filters.UserID, filters.APIKeyID, filters.AccountID, filters.GroupID, filters.Model, filters.RequestType, filters.Stream, filters.BillingType, filters.UpstreamModelMismatch)
		if err != nil {
			if !errors.Is(err, context.Canceled) && !errors.Is(err, context.DeadlineExceeded) {
	...
	runPaths := func(c context.Context) {
		res, err := r.getEndpointPathStatsWithFilters(c, start, end, filters.UserID, filters.APIKeyID, filters.AccountID, filters.GroupID, filters.Model, filters.RequestType, filters.Stream, filters.BillingType, filters.UpstreamModelMismatch)
		if err != nil {
			if !errors.Is(err, context.Canceled) && !errors.Is(err, context.DeadlineExceeded) {
...
func (r *usageLogRepository) getEndpointStatsByColumnWithFilters(ctx context.Context, endpointColumn string, startTime, endTime time.Time, userID, apiKeyID, accountID, groupID int64, model string, requestType *int16, stream *bool, billingType *int8, upstreamModelMismatch *bool) (results []EndpointStat, err error) {
	...
	query = appendUsageLogUpstreamModelMismatchQueryFilter(query, upstreamModelMismatch, "")
...
func (r *usageLogRepository) getEndpointPathStatsWithFilters(ctx context.Context, startTime, endTime time.Time, userID, apiKeyID, accountID, groupID int64, model string, requestType *int16, stream *bool, billingType *int8, upstreamModelMismatch *bool) (results []EndpointStat, err error) {
	actualCostExpr := "COALESCE(SUM(actual_cost), 0) as actual_cost"
	if accountID > 0 && userID == 0 && apiKeyID == 0 {
		actualCostExpr = "COALESCE(SUM(COALESCE(account_stats_cost, total_cost) * COALESCE(account_rate_multiplier, 1)), 0) as actual_cost"
	}
...
func scanUsageLog(scanner interface{ Scan(...any) error }) (*service.UsageLog, error) {
	var (
		...
		imageFirstOutputMs        sql.NullInt64
	...
	if imageFirstOutputMs.Valid {
		value := int(imageFirstOutputMs.Int64)
		log.ImageFirstOutputMs = &value
	}
	if userAgent.Valid {
