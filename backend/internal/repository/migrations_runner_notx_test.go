...
func TestApplyMigrationsFS_BatchImageIdempotencyUniqueMigration_DropsInvalidIndexBeforeRetry(t *testing.T) {
	...
	mock.ExpectQuery("SELECT checksum FROM schema_migrations WHERE filename = \\$1").
		...
		WillReturnError(sql.ErrNoRows)
	mock.ExpectQuery("SELECT EXISTS \\(").
		WithArgs(batchImageJobsOwnerIdempotencyUniqueIndex).
		WillReturnRows(sqlmock.NewRows([]string{"exists"}).AddRow(true))
	mock.ExpectExec("DROP INDEX CONCURRENTLY IF EXISTS batch_image_jobs_owner_idempotency_unique_idx").
		WillReturnResult(sqlmock.NewResult(0, 0))
	mock.ExpectExec("CREATE UNIQUE INDEX CONCURRENTLY IF NOT EXISTS batch_image_jobs_owner_idempotency_unique_idx").
		WillReturnResult(sqlmock.NewResult(0, 0))
	mock.ExpectExec("INSERT INTO schema_migrations \\(filename, checksum\\) VALUES \\(\\$1, \\$2\\)").
