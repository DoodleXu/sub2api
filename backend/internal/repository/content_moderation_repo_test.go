...
func TestContentModerationRepositoryEngineMetaInsert(t *testing.T) {
	for _, meta := range []*service.ContentModerationEngineMeta{nil, {Engine: "typesafe", Model: "jev-fixed", RulesVersion: "rules-v1", SkippedImages: 1}} {
		...
		if meta == nil {
			...
			args[33] = `{"engine":"typesafe","model":"jev-fixed","rules_version":"rules-v1","skipped_images":1}`
		}
...
func TestContentModerationRepositoryEngineMetaRead(t *testing.T) {
	for _, meta := range []any{nil, `{"engine":"typesafe","model":"jev-fixed","rules_version":"rules-v1","skipped_images":1}`} {
		...
		columns := []string{"id", "request_id", "user_id", "user_email", "api_key_id", "api_key_name", "group_id", "group_name", "endpoint", "provider", "model", "mode", "action", "flagged", "highest_category", "highest_score", "category_scores", "threshold_snapshot", "input_excerpt", "input_hash", "matched_keyword", "policy_id", "policy_action", "policy_snapshot", "block_status", "error_code", "upstream_latency_ms", "error", "violation_count", "auto_banned", "email_sent", "email_deduped", "last_email_sent_at", "status", "queue_delay_ms", "created_at", "engine_meta"}
		mock.ExpectQuery("SELECT[\\s\\S]*l.engine_meta").WillReturnRows(sqlmock.NewRows(columns).AddRow(1, "req", nil, "", nil, "", nil, "", "/v1/responses", "business", "gpt-model", "observe", "allow", false, "sexual", 0.1, `{"sexual":0.1}`, `{"sexual":0.8}`, "sample", "", "", nil, "", `{}`, 200, "", 10, "", 0, false, false, false, nil, "active", nil, time.Now(), meta))
...
func TestContentModerationRepositoryListLogs_ReturnsEmailDedupeStatus(t *testing.T) {
	...
	rows := sqlmock.NewRows([]string{
		...
		"violation_count", "auto_banned", "email_sent", "email_deduped", "last_email_sent_at", "user_status", "queue_delay_ms", "created_at", "engine_meta",
	}).AddRow(
		...
		1, false, false, true, lastEmailSentAt, "active", nil, createdAt, nil,
	)
