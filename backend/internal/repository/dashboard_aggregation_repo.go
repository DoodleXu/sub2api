...
const (
	usageBillingDedupCleanupBatchSize = 10000
	usageLogsCleanupBatchSize         = 10000
)
...
func (r *dashboardAggregationRepository) recomputeRangeInTx(ctx context.Context, hourStart, hourEnd, accountCostEnd, dayStart, dayEnd time.Time) error {
	...
	if _, err := r.sql.ExecContext(ctx, "DELETE FROM usage_dashboard_account_model_hourly WHERE bucket_start >= $1 AND bucket_start < $2", hourStart, hourEnd); err != nil {
		return err
	}
	...
	if _, err := r.sql.ExecContext(ctx, "DELETE FROM usage_dashboard_account_model_daily WHERE bucket_date >= $1::date AND bucket_date < $2::date", dayStart, dayEnd); err != nil {
		return err
	}
	if err := r.insertHourlyActiveUsers(ctx, hourStart, hourEnd); err != nil {
		return err
...
func (r *dashboardAggregationRepository) AggregateAccountCostRange(ctx context.Context, start, end time.Time) error {
	...
	run := func(repo *dashboardAggregationRepository) error {
		...
		if _, err := repo.sql.ExecContext(ctx, "DELETE FROM usage_dashboard_account_model_hourly WHERE bucket_start >= $1 AND bucket_start < $2", startLocal, endLocal); err != nil {
			return err
		}
		...
		if _, err := repo.sql.ExecContext(ctx, "DELETE FROM usage_dashboard_account_model_daily WHERE bucket_date >= $1::date AND bucket_date < $2::date", dayStart, dayEnd); err != nil {
			return err
		}
...
func (r *dashboardAggregationRepository) CleanupAggregates(ctx context.Context, hourlyCutoff, dailyCutoff time.Time) error {
	...
	if _, err := r.sql.ExecContext(ctx, "DELETE FROM usage_dashboard_account_model_hourly WHERE bucket_start < $1", hourlyCutoffUTC); err != nil {
		return err
	}
	...
	if _, err := r.sql.ExecContext(ctx, "DELETE FROM usage_dashboard_account_model_daily WHERE bucket_date < $1::date", dailyCutoffUTC); err != nil {
		return err
	}
...
func cleanupUsageLogsBatchWithRollupInvalidation(ctx context.Context, db *sql.DB, cutoff time.Time) (int64, error) {
	...
	rows, err := tx.QueryContext(ctx, `
		WITH victims AS (
			SELECT tableoid, ctid FROM usage_logs WHERE created_at < $1
	...
	for rows.Next() {
		...
		affected++
		if earliest.IsZero() || deleted.Before(earliest) {
			earliest = deleted
		}
	}
	...
	if affected > 0 {
		if err := invalidateGroupUsageRollupsAt(ctx, tx, earliest); err != nil {
			return rollback(err)
		}
	}
	if err := tx.Commit(); err != nil {
...
func (r *dashboardAggregationRepository) dropUsageLogsPartitions(ctx context.Context, cutoff time.Time) error {
	rows, err := r.sql.QueryContext(ctx, `SELECT c.relname FROM pg_inherits JOIN pg_class c ON c.oid = pg_inherits.inhrelid JOIN pg_class p ON p.oid = pg_inherits.inhparent WHERE p.relname = 'usage_logs'`)
	if err != nil {
		return err
	}
	...
	for rows.Next() {
		...
		if month.Before(cutoffMonth) {
			partitions = append(partitions, partition{name, month})
		}
	}
	if err := rows.Err(); err != nil {
		_ = rows.Close()
	...
	if db, ok := r.sql.(*sql.DB); ok {
		for _, p := range partitions {
			if err := dropUsageLogsPartitionWithRollupInvalidation(ctx, db, p.name, p.month); err != nil {
				return err
			}
		}
		return nil
	...
	for _, p := range partitions {
		if _, err := r.sql.ExecContext(ctx, fmt.Sprintf("DROP TABLE IF EXISTS %s", pq.QuoteIdentifier(p.name))); err != nil {
			return err
		}
	}
	return nil
...
func (r *dashboardAggregationRepository) upsertHourlyModelAggregates(ctx context.Context, start, end time.Time) error {
	...
	query := `
		...
		DO UPDATE SET
			...
			computed_at = EXCLUDED.computed_at
	`
	if _, err := r.sql.ExecContext(ctx, query, start, end, tzName); err != nil {
		return err
	}
	...
	accountModelQuery := `
		WITH hourly AS (
			...
			FROM usage_logs
			WHERE created_at >= $1 AND created_at < $2
			GROUP BY 1, account_id, 3
		)
		INSERT INTO usage_dashboard_account_model_hourly (
			bucket_start, account_id, model, standard_cost, computed_at
		)
		SELECT bucket_start, account_id, model, standard_cost, NOW()
		FROM hourly
		ON CONFLICT (bucket_start, account_id, model)
		DO UPDATE SET
			standard_cost = EXCLUDED.standard_cost,
			computed_at = EXCLUDED.computed_at
	`
	_, err := r.sql.ExecContext(ctx, accountModelQuery, start, end, tzName)
	return err
}
...
func (r *dashboardAggregationRepository) upsertDailyModelAggregates(ctx context.Context, start, end time.Time) error {
	...
	accountModelQuery := `
		WITH daily AS (
			...
			FROM usage_dashboard_account_model_hourly
			WHERE bucket_start >= $1 AND bucket_start < $2
			GROUP BY 1, account_id, model
		)
		INSERT INTO usage_dashboard_account_model_daily (
			bucket_date, account_id, model, standard_cost, computed_at
		)
		SELECT bucket_date, account_id, model, standard_cost, NOW()
		FROM daily
		ON CONFLICT (bucket_date, account_id, model)
		DO UPDATE SET
			standard_cost = EXCLUDED.standard_cost,
			computed_at = EXCLUDED.computed_at
	`
