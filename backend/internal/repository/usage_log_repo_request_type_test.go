...
func TestUsageLogRepositoryUpstreamModelMismatchFilterReachesAllAdminQueries(t *testing.T) {
	...
	t.Run("summary_and_endpoints", func(t *testing.T) {
		...
		mock.ExpectQuery("(?s)FROM usage_logs ul.*WHERE ul\\.upstream_model_mismatch IS TRUE.*GROUP BY GROUPING SETS").
			WithArgs(start, end).
...
func TestUsageLogRepositoryGetStatsWithFiltersRequestedModelSource(t *testing.T) {
	...
	mock.ExpectQuery("(?s)FROM usage_logs ul.*WHERE COALESCE\\(NULLIF\\(TRIM\\(ul\\.requested_model\\), ''\\), ul\\.model\\) = \\$1.*GROUP BY GROUPING SETS").
		WithArgs("gpt-5").
		WillReturnRows(sqlmock.NewRows([]string{
			"inbound_grouped",
			"upstream_grouped",
...
func TestUsageLogRepositoryGetStatsWithFiltersRequestTypePriority(t *testing.T) {
	...
	mock.ExpectQuery("(?s)FROM usage_logs ul.*WHERE \\(ul\\.request_type = \\$1 OR \\(ul\\.request_type = 0 AND ul\\.stream = FALSE AND ul\\.openai_ws_mode = FALSE\\)\\).*GROUP BY GROUPING SETS").
		WithArgs(requestType).
		WillReturnRows(sqlmock.NewRows([]string{
			"inbound_grouped",
			"upstream_grouped",
...
func TestUsageLogRepositoryGetStatsWithFiltersAlwaysReturnsAccountCost(t *testing.T) {
	...
	mock.ExpectQuery("(?s)FROM usage_logs ul.*GROUP BY GROUPING SETS").
		WillReturnRows(sqlmock.NewRows([]string{
			"inbound_grouped", "upstream_grouped", "inbound_endpoint", "upstream_endpoint",
			"requests", "input_tokens", "output_tokens", "cache_creation_tokens", "cache_read_tokens",
			"cost", "actual_cost", "account_cost", "real_cost_cny", "avg_duration_ms",
