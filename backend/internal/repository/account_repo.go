...
func createAccountRecord(ctx context.Context, client *dbent.Client, account *service.Account) error {
	...
	builder.SetTotalCostCny(account.TotalCostCNY)
	...
	if status != "" {
		...
		case service.StatusActive:
			q = q.Where(
				...
				rateLimitSchedulablePredicate(time.Now()),
		...
		case "rate_limited":
			q = q.Where(
				...
				dbaccount.Not(openAIContinueAfterLimitPredicate()),
				tempUnschedulablePredicate(),
			)
		case "temp_unschedulable":
			q = q.Where(
				dbaccount.StatusEQ(service.StatusActive),
				dbaccount.Not(tempUnschedulablePredicate()),
			)
		case "unschedulable":
			q = q.Where(
				dbaccount.StatusEQ(service.StatusActive),
...
// ListModelAvailabilityCandidates returns accounts whose persistent settings
// support model availability checks, ignoring transient runtime throttles.
func (r *accountRepository) ListModelAvailabilityCandidates(ctx context.Context, groupID *int64, platforms []string, includeGrouped bool) ([]service.Account, error) {
	...
	accounts, err := r.client.Account.Query().Where(preds...).Order(dbent.Asc(dbaccount.FieldPriority)).All(ctx)
	if err != nil {
		return nil, err
	}
	return r.accountsToService(ctx, accounts)
...
func (r *accountRepository) BulkUpdate(ctx context.Context, ids []int64, updates service.AccountBulkUpdate) (int64, error) {
	...
	if updates.TotalCostCNY != nil {
		setClauses = append(setClauses, "total_cost_cny = $"+itoa(idx))
		args = append(args, *updates.TotalCostCNY)
		idx++
	}
