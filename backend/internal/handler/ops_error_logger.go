...
// logOpsStreamError records in-band failures on 2xx responses, including non-streaming bodies.
// Request-scoped failures are independent of earlier upstream attempts.
...
func shouldSkipOpsErrorLog(ctx context.Context, ops *service.OpsService, message, body, requestPath string) bool {
	...
	// Check if disabled API key errors should be ignored
	if settings.IgnoreDisabledApiKeyErrors {
		if strings.Contains(bodyLower, opsErrAPIKeyDisabled) || strings.Contains(msgLower, "api key is disabled") {
			return true
		}
	}
