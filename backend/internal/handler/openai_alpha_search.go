...
func (h *OpenAIGatewayHandler) AlphaSearch(c *gin.Context) {
	...
	if decision := h.checkSecurityAudit(c, reqLog, apiKey, subject, service.ContentModerationProtocolOpenAIAlphaSearch, requestedModel, body); decision != nil && !decision.AllowNextStage {
		h.openAISecurityAuditError(c, decision)
		return
	}
	if h.rejectIfCyberSessionBlockedWithFallback(c, apiKey, body, requestedModel, cyberBlockFormatResponses, searchID) {
		return
	}
	...
	for {
		...
		if service.GetOpsCyberPolicy(c) != nil {
			cyberBlockKey = service.CyberSessionBlockKeyWithFallback(apiKey.ID, c, body, searchID)
		}
		h.recordCyberPolicyIfMarked(c, apiKey, account, subscription, requestedModel, err != nil, []byte(cyberBlockKey), channelMapping.ToUsageFields(requestedModel, ""), service.HashUsageRequestPayload(body))
