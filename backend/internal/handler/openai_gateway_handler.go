...
func (h *OpenAIGatewayHandler) Responses(c *gin.Context) {
	...
	for {
		// 流式请求会在客户端断开后继续排空上游响应；每次换号前重新检查，
		// 避免为已经取消的请求启动新的重放。
		...
		if err != nil {
			reqLog.Warn("openai.account_select_failed",
				openAIAccountSelectFailedLogFields(openAICompatibleSelectionErrorForLog(err, requestPlatform), len(failedAccountIDs), failoverAttempts)...,
			)
			...
			if result != nil && result.ImageCount > 0 {
				...
				if errors.As(err, &failoverErr) {
					...
					reqLog.Warn("openai.upstream_failover_switching",
						openAIFailoverSwitchLogFields(account.ID, failoverErr.StatusCode, switchCount, maxAccountSwitches, false, failoverAttempts)...,
					)
					continue
				}
...
func (h *OpenAIGatewayHandler) handleAnthropicFailoverExhausted(c *gin.Context, failoverErr *service.UpstreamFailoverError, streamStarted bool) {
	...
	if failoverErr.SuppressClientError {
		h.anthropicStreamingAwareError(c, http.StatusBadGateway, "api_error", "Upstream request failed", streamStarted)
		return
	}
...
//
// ws_v2 passthrough ingress 也会逐 turn 调用 BeforeTurn，确保长连接跨峰谷、
// 账号倍率刷新或余额变化后重新执行利润与计费资格复核。绝不能用建连时刻初始化：
// 那会把连接内所有 turn 钉死在旧的高峰因子上。AfterTurn 仍以 TurnStarted
// 记录的所属 turn 时刻作为防御性回退。
...
func (h *OpenAIGatewayHandler) ResponsesWebSocket(c *gin.Context) {
	...
	checkWSBillingEligibility := func() error {
...
func openAIFailoverSwitchLogFields(accountID int64, upstreamStatus int, switchCount int, maxAccountSwitches int, exhaustiveScheduler bool, attempts []openAIFailoverAttemptLog) []zap.Field {
	fields := []zap.Field{
		zap.Int64("account_id", accountID),
		zap.Int("upstream_status", upstreamStatus),
		zap.Int("switch_count", switchCount),
	}
	if exhaustiveScheduler {
		fields = append(fields, zap.Bool("openai_scheduler_exhaust_candidates", true))
	} else {
...
func (h *OpenAIGatewayHandler) ensureOpenAIStreamReadErrorResponse(c *gin.Context, err error, streamStarted bool) bool {
	...
	h.handleStreamingAwareErrorWithCode(c, http.StatusBadGateway, "upstream_error", code, message, streamStarted, true)
	return true
}
...
func closeOpenAIWSFailoverExhausted(args ...any) {
	var c *gin.Context
	...
	if failoverErr == nil {
		service.MarkOpsStreamFailure(c, "upstream_error", "server_error", "upstream websocket proxy failed", http.StatusBadGateway)
	...
	if failoverErr.Stage == service.GatewayFailureStageAccountAuth {
		status, message := credentialFailoverClientResponse(failoverErr)
		service.MarkOpsStreamFailure(c, "upstream_error", "upstream_error", message, status)
...
func (h *OpenAIGatewayHandler) rejectIfCyberSessionBlockedWithFallback(c *gin.Context, apiKey *service.APIKey, body []byte, model string, format cyberSessionBlockFormat, fallbackSessionID string) bool {
	...
	if h.gatewayService.FindCyberSessionBlockedKey(c.Request.Context(), key) == "" {
		return false
	}
...
func buildCyberSessionBlockWritePlan(apiKeyID int64, c *gin.Context, body []byte) cyberSessionBlockWritePlan {
	...
	// The non-explicit request path stores only the two newest transcript
	// prefixes; older prefixes are covered by the bounded lookup path and do
	// not belong in the write set for this request.
	if len(transcriptKeys) > 2 {
		transcriptKeys = transcriptKeys[len(transcriptKeys)-2:]
	}
...
func (h *OpenAIGatewayHandler) recordCyberPolicyIfMarked(c *gin.Context, apiKey *service.APIKey, account *service.Account, subscription *service.UserSubscription, model string, forwardErrored bool, cyberBlockBody []byte, channelFields service.ChannelUsageFields, requestPayloadHash string) {
	...
	if c.Request != nil {
		...
		if len(candidate) == 64 && strings.IndexFunc(candidate, func(r rune) bool {
			...
			// Keep the legacy direct-key contract used by callers that already
			// supplied a derived block key but have no API key context.
			cyberBlockKey = candidate
		} else {
			cyberBlockKey = service.CyberSessionBlockKeyWithFallback(apiKeyID, c, cyberBlockBody, sessionID)
		}
	}
	apiKeyPrefix := ""
	if apiKey != nil {
