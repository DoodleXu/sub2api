...
- OpenAI 账号编辑页新增账号级“达到限额后仍尝试调度”开关，持久化于 `extra.openai_continue_scheduling_after_limit`；该字段必须保留在调度快照白名单中，后续同步上游账号编辑器、调度查询或快照投影时不得丢失。
...
- 会话首请求在第一次选号前固定判定；存在 `previous_response_id`、既有粘性绑定或请求体中明确的多轮/工具调用历史视为后续请求。若账号 5h/7d 实际用量已达到 100% 且窗口尚未重置，新会话首请求必须跳过该账号；低于实际限额时，即使超过管理员预警阈值或刚收到瞬时 429，仍可正常承接首请求。该判定在同一请求的故障转移过程中不得因新建粘性绑定而变化。
- 该账号的上游账号级/服务级错误强制尝试其他可用账号，并标记为不可直接透传；若没有其他账号可用，用户侧只收到通用网关失败，不暴露该账号的原始上游响应。请求体过大等请求级错误不强制换号，已向客户端写出语义响应后的流式错误也不执行不安全重放。`/v1/messages/count_tokens` 作为无状态预检不创建粘性绑定；普通账号保持单账号错误契约，开启本开关的账号才会在失败后切换。
