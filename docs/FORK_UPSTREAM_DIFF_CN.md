...
- 备份恢复兼容旧记录：普通孤儿恢复任务缺少 `restore_started_at` 时回退使用 `started_at` 判断过期；归档恢复记录则建立新的保护窗口，避免迁移前旧记录被误清理。
- 内容审核保留语义与关键词两条输入视图：带 `<system-reminder>` 的语义片段整体排除，关键词检查保留提醒原文，避免本地关键词被提醒包裹时漏检；关键词命中优先于允许哈希。
- 管理设置保留 OAuth 调度倍率显式 `null` 与省略字段语义；网关失败响应保留机器可读状态；合成图片兼容测试使用有效最小 PNG；WS simple mode 在首帧和每轮结束复核计费窗口。
- 验证：备份恢复、提醒关键词和设置/WS/图片定向测试通过。后端服务全量 unit 仍有与本轮无关的计费、分组解析和错误别名历史失败，未宣称全量绿色。
...
- 插件管理菜单开关补齐管理员更新请求、保存响应、设置持久化及回读、公共设置和 HTML 注入链路；省略字段保留原值，默认关闭，仍仅控制菜单显示、不改变插件运行状态。补充开启/关闭/省略字段及公共设置注入回归测试。
- 发布候选检查补齐上游暂停账号 OAuth 刷新修复：手动暂停调度的 active 账号继续刷新凭据，保留已删除、已归档及母账号归档过滤，避免暂停期间令牌过期；数据库查询与上游服务及集成测试契约对齐。
...
- 管理端系统设置 API 对 GET/PUT 响应中的数组字段执行完整结构校验：`null` 或旧版本缺省字段继续归一化为空数组；错误容器或错误元素 fail closed，由设置页显示持久错误、禁用全量保存，运营中心在设置未成功加载时同样禁止保存签到规则，避免把畸形响应静默转换为空配置后覆盖支付方式、自定义端点等既有数据。
- 网页工作台端点和支付类型计算保留局部防御；动态路由 chunk 首次加载失败继续自动刷新，短时间内再次失败则进入随主包加载的可见恢复页。恢复目标通过 URL 解析后严格校验同源，阻断反斜杠绕过的开放重定向；恢复页加入 backend mode 匿名白名单，确保登录 chunk 失败时也能显示。
- 验证结果：前端 typecheck、相关文件 ESLint、定向 Vitest（5 个文件、105 个用例）、全量 Vitest（2014/2014）和生产构建通过，`git diff --check` 通过；本地浏览器验证恢复页桌面/390px 移动端布局、同源返回和反斜杠跨域地址回退均正常。未执行生产部署、远程推送、tag 或 release。
...
## 2026-08-31 放开 API Key 自助换组

- 用户自助编辑 API Key 允许在订阅分组与非订阅/钱包分组之间切换；切换到非订阅分组时自动清除 `subscription_id`，仍保留目标分组权限校验。
- 删除 `API_KEY_SUBSCRIPTION_SWITCH_BLOCKED` 自助拦截及其告警逻辑；订阅升级订单的额度折抵和履约风控保持不变。
...
- 2026-08-15 安全收紧：移除匿名 `out_trade_no` 订单状态查询及支付结果页的匿名回退，仅保留登录态订单查询和签名 resume token 恢复；同时升级前端内容净化与构建依赖，消除 DOMPurify、Mermaid、PostCSS 已知漏洞。

- 2026-08-15 支付看板的 7/30/90 天筛选改为点击后显式加载，并丢弃过期请求响应，确保快速切换时统计数据不会回写到错误的筛选范围；后端在 `days` 与全局 GET `timezone` 参数同时存在时优先采用明确的天数，避免 `timezone` 误触发默认 30 天日期范围。筛选按钮继续保留 fork 的多币种支付统计展示。
...
- 发版候选修复：OpenAI 用量统一计费入口将 API Key 的完整分组传入定价解析器，确保分组长上下文开关与账号开关必须同时允许；不再因只传 group ID 而把已关闭的分组错误当作默认开启。
...
- 额外覆盖“分组关闭、账号开启”不能应用长上下文阶梯的计费组合语义。
...
- 2026-08-11 审核修复：已绑定或既有邮箱的 OAuth 登录不再执行“新用户注册”域名策略；只有实际创建用户时才走同一原子限额写入守卫，避免策略变更将存量用户锁在登录之外。
...
- Release 增加严格且兼容 Docker 的 tag 格式、annotated tag 类型、非空发布说明、tag 版本与目标提交 `backend/cmd/server/VERSION` 一致性、默认分支祖先关系以及精确 SHA 的 CI/Security Scan 成功校验；手动 tag 输入统一通过环境变量传入 shell，发布与前端构建固定检出同一已验证 SHA，发布前再次确认 annotated tag 未被移动。版本必须在打 tag 前提交，Release 不生成 VERSION 产物，也不在发布后反向改写默认分支。
- 2026-08-11 审核修复：新增 `tools/verify_release_candidate.sh`，在创建 annotated tag 前按候选提交的精确 SHA 校验 `backend-ci.yml` 与 `security-scan.yml` 已成功，并验证 VERSION 与目标 tag 一致；不可再将“先打 tag、后发现检查失败”作为正常发布路径。
...
- standalone Compose 示例补齐外部 PostgreSQL/Redis 必填变量；迁移文档统一为不可变、forward-only 和补偿迁移/备份恢复语义。并发测试在 `TestMain` 一次性设置 Gin 全局模式，AST 回归测试禁止 service 包中的并行测试再次修改该进程级状态；异步 dashboard 测试桩改用原子计数，恢复批量账号负载集成断言。账号管理的大型交互弹窗改为按需加载，主视图生产 chunk 从约 678 KB 降至约 158 KB。
...
- 用户和 API Key 管理写路径改为显式列掩码，余额调整使用原子接口，避免并发计费、批量限额或状态更新被旧快照覆盖；API Key 编辑继续使用 fork 的分组/订阅绑定解析，名称在创建和编辑时均按原值存储，由前端文本插值负责安全渲染，避免实体显示和重复编码。订阅 ID 仅随绑定字段更新，不会在编辑其他列时被旧快照覆盖。
...
- Live observer 同时保留 fork 拨号重试期间续租、租约丢失终结和创建失败补偿关闭，并吸收上游 store 抖动有限重试、claim 错误到期兜底 finalize；终态写入以 usage log 持久层唯一约束实现跨实例幂等，不再依赖 Redis closed 标记作为唯一落库机会，Redis 恢复后有限重试 closed 标记与租约释放。
...
- 后端 `TZ=UTC go test -tags=unit -count=1 ./...`、`TZ=UTC go vet -tags=unit ./...` 与 `TZ=UTC go test -race -tags=unit -count=1 ./internal/service` 通过；定向覆盖 Passkey、模型广场、设置聚合、用户/API Key 按列更新和名称重复编辑、原子余额、Codex manifest、Claude cache breakpoint 和 Live 生命周期。
...
- Release tag 校验脚本覆盖有效 annotated tag，并拒绝轻量 tag、Docker 不兼容的 build metadata、提交版本不一致、空发布说明和非默认分支祖先；`actionlint v1.7.12` 对 Backend CI 与 Release workflow 校验通过。
...
- OpenAI 账号编辑页新增账号级“达到限额后仍尝试调度”开关，持久化于 `extra.openai_continue_scheduling_after_limit`；该字段必须保留在调度快照白名单中，后续同步上游账号编辑器、调度查询或快照投影时不得丢失。开关开启且账号未被其他健康门禁阻断时，管理端统一显示“超频中”，不再显示“限流中”、恢复倒计时或 429 徽标，并将账号归入可调度的活跃状态筛选。
- 开启后仅绕过限额类阻断：5h/7d 自动暂停、API Key 总量/日/周本地额度、账号级 `rate_limit_reset_at`、模型级限流和 429 内存冷却；账号自定义的 429 临时不可调度规则也不能覆盖本开关。手动暂停、归档、过期、401/403、过载、非 429 临时不可调度、代理隔离及请求级错误继续按原规则阻断，避免把凭据或传输故障误当作配额限制。
- 开启该开关的账号组成独立“超频优先层”：每个请求第一次选号时只读取一次候选账号池，并划分为超频与普通两层；存在可用超频账号时先按原调度策略选择一个账号，该优先级高于 `previous_response_id`、会话粘性、静态优先级和普通账号评分。若所选超频账号失败，本次请求后续选号排除整个超频层并直接回退到普通健康账号，避免在超频账号之间连续消耗故障转移次数；Grok 等非 OpenAI 平台不执行超频预选。
- 超频账号的上游账号级/服务级错误强制尝试普通账号，并标记为不可直接透传；普通账号随后成功时用户无感，普通账号仍失败时按普通账号的最终错误响应。若没有普通账号可用，用户侧只收到通用网关失败，不暴露超频账号的原始上游响应。请求体过大等请求级错误不强制换号，已向客户端写出语义响应后的流式错误也不执行不安全重放。`/v1/messages/count_tokens` 作为无状态预检不创建粘性绑定；普通账号保持原有错误契约。
...
- OpenAI 自动低倍率调度对明确不支持倍率探测的 API Key 账号，改用账号“每刀成本”（累计人民币成本 / 累计标准计费）参与排序和高级成本权重计算，并按可热更新的“调度美元兑人民币基准汇率”（默认 `7.2`）换算为与上游倍率一致的无量纲成本；汇率变化会触发调度快照重建。未探测、过期或无有效成本样本的账号仍保持未知，不改变既有兜底顺序。仅在成本策略生效时为调度快照批量加载成本统计，策略热切换会异步请求快照重建；统计失败时复用缓存中的标准计费分母并结合账号最新人民币成本重新计算，继续调度且不发布错误零值，避免请求热路径扫描用量日志或因统计查询异常阻断账号选择。
...
- 管理员保存单个或批量账号的 `total_cost_cny`（含 API Key 增量成本）后，立即废弃 Redis 成本摘要，并在 2 秒 debounce 窗口内合并重算单行成本快照；无需重扫 `usage_logs`，首页“今日人民币实际成本”不会继续显示保存前的账号成本。
...
- 已追平账本上的单行 usage 删除、成本字段修正和跨账号移动直接按 OLD/NEW 值增量修正账本；未处理尾部只标记 pending，只有部分账本等不确定状态才回退整账号重建，避免常规历史修正重扫账号全部 usage。
- Dashboard 首次/迁移回填使用 watermark 上的持久化 cursor、target 和 running 状态按日推进；实时调度遇到进行中的历史回填会跳过重复尾部扫描，重启或下一轮 worker 可从断点继续。
- Dashboard 成本快照保留归档/软删除账号的历史累计成本口径；当前活动账号过滤继续由运维与调度类指标单独负责，避免账号归档导致历史成本从 Dashboard 消失。
...
- 归档账号不删除累计账本；归档且无新增 usage 的账号不会再次扫描，晚到 usage 由数据库触发器重新标记待核算。成本聚合和 Dashboard retention 对原始 `usage_logs` 严格只读，不会因为核算、回填或成本快照刷新而清理历史明细。管理员显式清理、单条删除或成本字段修正由触发器优先按差额更新账本，并把受影响小时写入 `usage_account_cost_dirty_buckets` 异步重算；只有无法安全计算差额的部分账本状态才回退精确重建。后台普通账号删除沿用 `SoftDeleteMixin`，保留该账号的 usage 与累计账本，确保历史成本不丢失；只有绕过软删除的显式物理删除才由外键级联清理 usage 与账本。
...
- 用户侧前端 `/console` 仅支持通过 OpenAI-compatible `/v1` 异步 Images 接口生图和本地会话存储；对话模式已移除。
...
- `frontend/src/features/web-console/utils.ts`
- `frontend/src/features/web-console/storage.ts`
- `frontend/src/api/asyncImageTasks.ts`
...
- `backend/internal/service/image_storage_browser.go`
...
历史归档实现提交线索（仅用于识别遗留表和旧数据，不应恢复为运行时双轨）：
...
- 上游异步图片任务、对象存储结果格式或路由变化时，Web 创作台和生图管理必须同步跟随同一契约，禁止重新引入第二套任务表、执行器或本地归档。
- 排障优先查 `image_task:*` Redis 任务、异步图片桶对象和 `image_task.offload_*` 日志；历史 `web_console_image_tasks` 与归档资产记录不再代表当前运行状态。
...
- 生图链路收口：Web 创作台统一使用上游异步 Images API，不再维护 fork 自建任务执行器和双轨归档运行时；管理端改为只读浏览异步图片前缀中的对象，保留前缀隔离、分页、预览和下载能力。
- Web Console 可靠性：生成结果写入浏览器 Cache Storage；对象存储未配置 CORS 时降级为预签名直链展示，且不把签名持久化到本地会话。部署文档补充桶 `GET`/`HEAD` CORS 要求。
- 调度成本口径：明确不支持上游倍率探测的 API Key 账号继续使用每美元人民币成本，但先按可热更新的美元兑人民币基准汇率换算为与上游倍率一致的无量纲值；默认汇率为 `7.2`，修改后触发调度快照重建。
...
- Web 创作台：生图、任务轮询、刷新恢复、会话删除；对话模式已移除。
