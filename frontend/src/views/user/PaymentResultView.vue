        ...
        <div v-if="order" class="rounded-xl bg-white p-5 shadow-sm dark:bg-dark-800">
          <div class="space-y-3 text-sm">
            ...
            <div v-if="feeAmount > 0" class="flex justify-between">
              <span class="text-gray-500 dark:text-gray-400">{{ feeAmountLabel }}</span>
...
const feeAmountLabel = computed(() => {
  if (!hasAmountFields(order.value) || order.value.fee_rate <= 0) return t('payment.orders.fee')
  return `${t('payment.orders.fee')} (${order.value.fee_rate}%)`
})
