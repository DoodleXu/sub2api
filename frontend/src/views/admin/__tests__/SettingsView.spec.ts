...
vi.mock("vue-i18n", async () => {
  ...
  const translations: Record<string, string> = {
    ...
    "admin.settings.openaiAdvancedScheduler.title": "OpenAI 高级调度",
    "admin.settings.openaiAdvancedScheduler.description": "保留原版高级调度开关。仅在老调度方案下启用原有的高级调度行为；试验性调度由下方调度方案独立控制。",
...
describe("admin SettingsView payment visible method controls", () => {
  ...
  it("places and explains rate controls for both scheduling modes", async () => {
    ...
    expect(priorityModeText.indexOf("OAuth 调度参考倍率")).toBeLessThan(
      priorityModeText.indexOf("OpenAI 高级调度"),
    );
