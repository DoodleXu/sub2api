...
import { escapeSpreadsheetCSVValue } from '@/utils/csv'
...
const exportToCSV = async () => {
  ...
  try {
    ...
    const csvChunks = [headers.map(escapeSpreadsheetCSVValue).join(',') + '\r\n']
    while (true) {
      ...
      if (rows.length) {
        csvChunks.push(rows.map(row => row.map(escapeSpreadsheetCSVValue).join(',')).join('\r\n') + '\r\n')
      }
    ...
    if(!c.signal.aborted) {
      saveAs(
        new Blob(['\uFEFF', ...csvChunks], { type: 'text/csv;charset=utf-8' }),
