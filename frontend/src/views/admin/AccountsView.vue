                    ...
                    <div class="overflow-y-auto p-2" :style="{ maxHeight: `${accountToolsDropdownPosition.maxHeight}px` }">
                      ...
                      <div class="space-y-2 px-3 py-2">
                        <div class="flex min-w-0 items-center justify-between gap-3">
                          <span class="min-w-0 flex-1 break-words text-sm font-medium text-gray-700 dark:text-gray-200">
                        ...
                        <div class="flex min-w-0 items-center gap-2">
                          <label class="min-w-0 flex-1 break-words text-xs text-gray-500 dark:text-gray-400" for="upstream-billing-probe-interval">
                          ...
                          <input
                            ...
                            min="5"
                            max="1440"
                            class="input h-8 w-20 shrink-0 px-2 text-sm"
                          />
                          <button
                            type="button"
                            class="btn btn-secondary h-8 shrink-0 px-2"
      ...
      <template #table>
        ...
        >
          ...
          <template #header-upstream_billing_rate="{ column }">
            <div data-test="upstream-billing-header" class="flex items-center">
              <span>{{ column.label }}</span>
              <HelpTooltip data-test="usage-windows-hint" :content="t('admin.accounts.upstreamBilling.trustWarning')" width-class="w-80" />
            </div>
          </template>
...
import type { Account, AccountListItem, AccountPlatform, AccountSchedulerGroupScore, AccountType, AccountUsageInfo, Proxy as AccountProxy, AdminGroup, WindowStats, ClaudeModel, UpstreamBillingProbeSettings, UpstreamBillingProbeSnapshot } from '@/types'

// Account editors and diagnostic dialogs are large and only needed after an
// explicit user action. Keep the table route interactive while these chunks
// load independently on first use.
...
const DEFAULT_HIDDEN_COLUMNS = ['today_stats', 'proxy', 'notes', 'rate_multiplier', 'scheduler_score']
...
const clearPendingListSyncTimer = () => {
  if (pendingListSyncTimer) {
    globalThis.clearTimeout(pendingListSyncTimer)
...
const syncPendingListChanges = async () => {
  if (isUnmounted || typeof window === 'undefined') return
  clearPendingListSyncTimer()
...
const schedulePendingListSync = () => {
  if (isUnmounted || typeof window === 'undefined' || !hasPendingListSync.value) return
  clearPendingListSyncTimer()
  pendingListSyncTimer = window.setTimeout(() => {
    syncPendingListChanges()
  }, 350)
...
function grokPersistedQuotaSnapshot(extra: Record<string, any>): Record<string, any> | undefined {
  const snapshot = extra.grok_usage_snapshot ?? extra.grok_quota_snapshot
...
function grok45ResponsesPlanIsHeavy(snapshot: Record<string, any> | undefined): boolean {
  ...
  const observedAt = Date.parse(String(snapshot.last_headers_seen_at || snapshot.updated_at || ''))
  const age = Date.now() - observedAt
  const model = String(snapshot.model || '').trim().toLowerCase().replace(/^(x-ai|xai)\//, '')
  const isFresh = Number.isFinite(observedAt) && age <= GROK_QUOTA_SIGNAL_MAX_AGE_MS && age >= -GROK_QUOTA_SIGNAL_MAX_FUTURE_SKEW_MS
  const heavyWindow = Number(snapshot.requests?.limit ?? 0) >= 8300 || Number(snapshot.tokens?.limit ?? 0) >= 53_000_000
  return normalizeGrokPlanKey(snapshot.plan_from_45_responses) === 'supergrokheavy' ||
    (isFresh && (model === 'grok-4.5' || model.startsWith('grok-4.5-')) && heavyWindow)
}

// JWT / unambiguous credentials outrank snapshots. SuperGrokPro is ambiguous
// and can only be upgraded to Heavy by a fresh Grok 4.5 Responses quota signal.
function getAccountPlanType(row: any): string | undefined {
  ...
  if (row.platform === 'grok') {
    ...
    if (grok45ResponsesPlanIsHeavy(grokPersistedQuotaSnapshot(extra)) &&
...
function getOpenAIAuthMode(row: any): string | undefined {
  ...
  const authMode = row.credentials?.auth_mode || row.parent_auth_mode
...
const allColumns = computed(() => {
  ...
  c.push({ key: 'usage', label: t('admin.accounts.columns.usageWindows'), sortable: false, class: 'min-w-[250px]' })
  c.push(
