            ...
            <div class="space-y-5 p-6">
              ...
              >
                <div class="min-w-0">
                  <label
                    class="text-sm font-medium text-gray-700 dark:text-gray-300"
                    for="openai-scheduling-usd-to-cny-rate"
                  >
                    {{ t("admin.settings.openaiExperimentalScheduler.usdToCnyRateTitle") }}
                  </label>
                  <p class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">
                    {{ t("admin.settings.openaiExperimentalScheduler.usdToCnyRateDescription") }}
                  </p>
                </div>
                <div class="relative w-full shrink-0 sm:w-32">
                  <input
                    id="openai-scheduling-usd-to-cny-rate"
                    v-model.number="form.openai_scheduling_usd_to_cny_rate"
                    class="input pr-12"
            ...
            <div class="space-y-6 p-6">
              ...
              <div>
                <label
                  class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300"
                >
                  {{ t("admin.settings.site.siteLaunchAt") }}
                </label>
                <input
                  v-model="form.site_launch_at"
                  type="datetime-local"
                  class="input font-mono text-sm"
                />
                <p class="mt-1.5 text-xs text-gray-500 dark:text-gray-400">
                  {{ t("admin.settings.site.siteLaunchAtHint") }}
                </p>
              </div>
...
const paymentGuideHref = computed(() =>
  locale.value.startsWith("zh")
    ? "https://github.com/DoodleXu/sub2api/blob/main/docs/PAYMENT_CN.md"
    : "https://github.com/DoodleXu/sub2api/blob/main/docs/PAYMENT.md",
);

const paymentMethodsHref = computed(() =>
  locale.value.startsWith("zh")
    ? "https://github.com/DoodleXu/sub2api/blob/main/docs/PAYMENT_CN.md#支持的支付方式"
    : "https://github.com/DoodleXu/sub2api/blob/main/docs/PAYMENT.md#supported-payment-methods",
);
...
function normalizeSiteLaunchAtForSave(value: string): string {
...
async function loadSettings() {
  ...
  try {
    ...
    form.site_launch_at = formatDateTimeLocal(settings.site_launch_at);
...
async function saveSettings() {
  ...
  try {
    ...
    const payload: UpdateSettingsRequest = {
      ...
      site_launch_at: normalizeSiteLaunchAtForSave(form.site_launch_at),
