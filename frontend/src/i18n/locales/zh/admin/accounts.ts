export default {
    accounts: {
      ...
      upstreamBilling: {
        trustWarning: '此倍率来自上游站点；Sub2API 原生上游使用 API Key 自省声明，New API 兼容上游使用最近消费日志中的实际分组倍率。Sub2API 无法独立验证该值是否与最终扣费一致，请结合账单、余额变化和实际用量自行核验。',
        autoProbeSettings: '自动倍率探测',
        intervalMinutes: '探测间隔（分钟）',
        settingsSaved: '自动探测设置已保存',
        settingsFailed: '保存自动探测设置失败',
        ...
        unsupported: '不支持',
        failed: '失败',
        noBillingSample: '暂无计费样本',
        notProbed: '未探测',
        newAPISource: '来源：New API 最近消费日志（观测值）',
        observedGroupRate: '最近消费分组：{group}，倍率 {value}x',
        observedRate: '观测倍率：{value}x',
        sampleAt: '样本时间：{value}',
      ...
      grokClientToolCache: {
        ...
        hint: '仅对已识别为 Free 的 Grok OAuth 账号生效；开启后为 Codex、Trae 等客户端函数工具请求启用上游提示缓存，可能加入原生搜索工具并改变自动工具选择。'
      },
