...
本 fork 的定制功能与上游官方仓库差异维护在 [Fork 与上游功能差异维护台账](docs/FORK_UPSTREAM_DIFF_CN.md)，用于后续同步上游、迭代和 debug。
...
```bash
curl -sSL https://raw.githubusercontent.com/DoodleXu/sub2api/main/deploy/install.sh | sudo bash
```
...
# 卸载
curl -sSL https://raw.githubusercontent.com/DoodleXu/sub2api/main/deploy/install.sh | sudo bash -s -- uninstall -y
```
...
# 下载并运行部署准备脚本
curl -sSL https://raw.githubusercontent.com/DoodleXu/sub2api/main/deploy/docker-deploy.sh | bash
...
```bash
# 1. 克隆仓库
git clone https://github.com/DoodleXu/sub2api.git
cd sub2api/deploy
...
- `gateway.upstream_response_read_max_bytes`：限制非流式上游响应读取大小（默认 `128MB`），兼顾 4K 图片的 base64 响应与异常响应内存保护。
...
<a href="https://star-history.dera.page/#DoodleXu/sub2api&Date">
 <picture>
   <source media="(prefers-color-scheme: dark)" srcset="https://star-history.dera.page/svg?repos=DoodleXu/sub2api&type=Date&theme=dark" />
   <source media="(prefers-color-scheme: light)" srcset="https://star-history.dera.page/svg?repos=DoodleXu/sub2api&type=Date" />
   <img alt="Star History Chart" src="https://star-history.dera.page/svg?repos=DoodleXu/sub2api&type=Date" />
 </picture>
</a>
